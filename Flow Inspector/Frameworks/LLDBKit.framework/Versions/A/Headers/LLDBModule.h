/*
 Copyright 2018 The Octadero Authors. All Rights Reserved.
 Created by Volodymyr Pavliukevych on 2017.
 Licensed under the GPL License, Version 3.0 (the "License");
 you may not use this file except in compliance with the License.
 You may obtain a copy of the License at
 
 http://www.gnu.org/licenses/gpl-3.0.txt
 
 Unless required by applicable law or agreed to in writing, software
 distributed under the License is distributed on an "AS IS" BASIS,
 WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 See the License for the specific language governing permissions and
 limitations under the License.
 */

#pragma once
#import "LLDBObject.h"
#import "LLDBDeclarations.h"

@interface LLDBModule : LLDBObject
@property (readonly,nonatomic,assign) NSUInteger numberOfCompileUnits;
@property (readonly,nonatomic,assign) size_t numberOfSymbols;

- (instancetype)init UNIVERSE_UNAVAILABLE_METHOD;
- (LLDBFileSpec*)fileSpec;
- (LLDBSymbolContextList *)findSymbols:(NSString *)name;
- (LLDBSection *)findSection:(NSString *)name;
- (LLDBValueList *)findGlobalVariables:(NSString *)name forTarget:(LLDBTarget *)target;
- (LLDBSymbolContextList *) findFunctions:(NSString *) name;
- (LLDBSymbol*)compileUnitAtIndex:(NSUInteger)index;
- (LLDBSymbol*)symbolAtIndex:(size_t)index;
- (void) resolveSymbolContextForAddress:(LLDBAddress *) address;
- (LLDBAddress *) resolveForAddress:(uint64_t) address;
- (BOOL)isEqualToModule:(LLDBModule*)object;
- (BOOL)isEqual:(id)object;
- (NSString *)description;
- (LLDBType *) findFirstType:(NSString *) typeName;
- (NSArray<LLDBType *> *) types;
@end


